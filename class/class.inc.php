...
class SqlIQuery {
	...
	public function isLogIn(){	//middleware function, redirect to login page if not log in
	...
	public function isLogOut(){		//redirect to dash board if already login
	...
	public function isAdmin(){
		if(isset($_SESSION['type'])){
			if(!($_SESSION['type'] === '1111')){
				echo "Error code 404, page not found!";
				exit;
			}
		}
	}
	...
	public function login($username, $password) {
		...
		$sql = "SELECT user_id, username, first_name, last_name, type, image, last_login FROM users WHERE username = BINARY '" . $this->db->escape($username) . "' AND password = BINARY '". $this->db->escape(md5($password)) . "' AND salt = BINARY '" . $this->db->escape($salt) . "' AND status = '1'";
    	...
    	if ($user_query->num_rows) {
			...
			if(empty($user_query->row['image']) && ($user_query->row['type'] == '2222')){				
			...
			$sql = "UPDATE users SET last_login = NOW() WHERE user_id = '" . (int)$_SESSION['user_id']. "'";
	...
	public function updateUserImage($image) {
		$sql = "UPDATE users SET image = '".$this->db->escape($image)."' WHERE user_id = '".$_SESSION['user_id']."'";
		$user_query = $this->db->query($sql);
	}


	public function getExamsWithAdmitcard() {
		$sql = "SELECT * FROM exams WHERE admitcard_date IS NOT NULL;";
		$user_query = $this->db->query($sql);
		return $user_query->rows;
	}

	public function getActiveExamApplications() {
		$sql = "SELECT * FROM exams WHERE application_status = '1'";
		$user_query = $this->db->query($sql);
		return $user_query->rows;
	}

	public function createNotification($data) {
		$sql = "INSERT INTO notifications SET text = '".$data['notification_text']."'";
	...
	public function isCurrentPassword($current_password) {//#
		$salt = substr(sha1($current_password), 0, 9);
		$sql = "SELECT user_id FROM users WHERE username = BINARY '" .$_SESSION['username']. "' AND password = BINARY '". $this->db->escape(md5($current_password)) . "' AND salt = BINARY '" . $this->db->escape($salt) . "' AND status = '1'";
		$user_query = $this->db->query($sql);
		if(empty($user_query->row)){
			return false;
		}else{
			return true;
		}
	...
	public function changePassword($new_password) {
		$salt = substr(sha1($new_password), 0, 9);
		echo $sql = "UPDATE users SET password = '".$this->db->escape(md5($new_password))."', 
		salt = '".$this->db->escape($salt)."' WHERE username = BINARY '".$_SESSION['username']."'";
		$user_query = $this->db->query($sql);
		if(empty($user_query->row)){
			return false;
		}else{
	...
	public function isExistSchoolCode($school_code) {		
		$sql = "SELECT COUNT(*) AS total FROM schools WHERE school_code = '".$school_code."'";
		$query = $this->db->query($sql);		
		return $query->row['total'];
	}
	
	public function addSchool($data) {		
		$sql = "INSERT INTO schools SET school_code = '" . $this->db->escape($this->utf8_str_trim($data['school_code'])) . "', school_name = '" . $this->db->escape($this->utf8_str_trim($data['school_name'])) . "', add_date = NOW()";
	    $this->db->query($sql);
	}
	
	public function getAllSchools() {
		$sql = "SELECT school_id, school_name, school_code FROM schools ORDER BY school_name";
		$query = $this->db->query($sql);
		return $query->rows;
	}

	public function isExistDepartmentCode($department_code) {		
		$sql = "SELECT COUNT(*) AS total FROM departments WHERE department_code = '".$department_code."'";
		$query = $this->db->query($sql);		
		return $query->row['total'];
	}

	public function addDepartment($data) {		
		$sql = "INSERT INTO departments SET department_code = '" . $this->db->escape($this->utf8_str_trim($data['department_code'])) . "', department_name = '" . $this->db->escape($this->utf8_str_trim($data['department_name'])) . "', school_id = '" . $this->db->escape($this->utf8_str_trim($data['school_id'])) . "', add_date = NOW()";
	    $this->db->query($sql);
	}

	public function isExistProgrammeCode($program_code) {		
		$sql = "SELECT COUNT(*) AS total FROM programs WHERE program_code = '".$program_code."'";
		$query = $this->db->query($sql);		
		return $query->row['total'];
	}

	public function getAllDepartments() {
		$sql = "SELECT department_id, department_name, department_code FROM departments ORDER BY department_name";
		$query = $this->db->query($sql);
		return $query->rows;
	}

	public function addProgram($data) {		
		$sql = "INSERT INTO programs SET program_code = '" . $this->db->escape($this->utf8_str_trim($data['program_code'])) . "', program_name = '" . $this->db->escape($this->utf8_str_trim($data['program_name'])) . "', department_id = '" . $this->db->escape($this->utf8_str_trim($data['department_id'])) . "', no_of_semester = '" . (int)$this->db->escape($this->utf8_str_trim($data['no_of_semester'])) . "', fee_per_semester = '" . $this->db->escape($this->utf8_str_trim($data['fee_per_semester'])) . "', add_date = NOW()";
	    $this->db->query($sql);
	}

	public function getAllPrograms() {
		$sql = "SELECT program_id, program_name, program_code FROM programs ORDER BY program_name";
		$query = $this->db->query($sql);
		return $query->rows;
	}

	public function getAllExams() {
		$sql = "SELECT DATE_FORMAT(exam_date,'%d-%M-%Y') as exam_date, exam_id, created_at FROM exams ORDER BY exam_date ASC";
		$query = $this->db->query($sql);
		return $query->rows;
	}

	public function getApplicationId($exam_id){		
		$date = date('Y-m-d');
		$sql = "SELECT DATE_FORMAT(exam_date,'%d-%M-%Y') as exam_date, exam_id FROM exams where end_admitcard_date >= ".$date." AND exam_id = ".$exam_id;
		if($this->db->query($sql)){
			$sql = "SELECT id from exam_applications WHERE exam_id = ".$this->db->escape($this->utf8_str_trim($exam_id))." AND user_id = ".$_SESSION['user_id'];
			$query = $this->db->query($sql);
			return $query->rows;
		}
	}

	public function getAllActiveAdmitcardExams() {
		$date = date('Y-m-d');
		$sql = "SELECT DATE_FORMAT(exam_date,'%d-%M-%Y') as exam_date, exam_id FROM exams where end_admitcard_date >= ".$date;
		$query = $this->db->query($sql);
		return $query->rows;
	}
	public function getAllExamsByYear($year) {
		$sql = "SELECT exam_id, exam_date, created_at FROM exams where YEAR(exam_date) =".$this->db->escape($year)." ORDER BY exam_id ASC";
		$query = $this->db->query($sql);
		return $query->rows;
	}

	public function getExamById($exam_id) {
		$sql = "SELECT exam_date, created_at, start_admitcard_date, end_admitcard_date  FROM exams where exam_id =".$this->db->escape($exam_id);
		$exam = $this->db->query($sql);
		if($exam->rows){
			$sql = "SELECT program_id, prog_semester FROM exam_structure where exam_id =".$this->db->escape($exam_id);
			if($exam_structure = $this->db->query($sql)){
				...
				foreach($ar as $a){
					...
					$sql = "SELECT program_id, program_name, program_code, no_of_semester FROM programs where program_id =".$this->db->escape($a['program_id']);
					if($program = $this->db->query($sql)){
						$program = $program->row;
						$n[$a['program_id']]['program_id'] = $program['program_id'];
						$n[$a['program_id']]['program_name'] = $program['program_name'];
						$n[$a['program_id']]['program_code'] = $program['program_code'];
						$n[$a['program_id']]['no_of_semester'] = $program['no_of_semester'];
					}
				}
				foreach($ar as $a){
				...
				$n['exam_create_details']['exam_date'] = $exam->row['exam_date'];
				$n['exam_create_details']['created_at'] = $exam->row['created_at'];
				$n['exam_create_details']['exam_id'] = $exam_id;
				$n['exam_create_details']['start_admitcard_date'] = $exam->row['start_admitcard_date'];
				$n['exam_create_details']['end_admitcard_date'] = $exam->row['end_admitcard_date'];
			}
			return $n;
		}else{
			return false;
	...
	public function getSemsById($program_id) {
		$sql = "SELECT no_of_semester FROM programs WHERE program_id = $program_id";
		$query = $this->db->query($sql);
		return $query->row['no_of_semester'];
	}

	public function getExamYears() {
		$sql = "SELECT DISTINCT DATE_FORMAT(exam_date,'%Y') as year FROM exams";
		$query = $this->db->query($sql);
		return $query->rows;
	}
	...
	public function addUser($data) {
		...
		username = '" . $this->db->escape($this->utf8_str_trim($data['username'])) . "', 
		first_name = '" . $this->db->escape($this->utf8_str_trim($data['first_name']))."', 
		last_name = '" . $this->db->escape($this->utf8_str_trim($data['last_name']))."', 
		email_id = '" . $this->db->escape($this->utf8_str_trim($data['email']))."',
		contact_no = '" . $this->db->escape($this->utf8_str_trim($data['contact_no']))."', 
		type = '" . $this->db->escape($this->utf8_str_trim('2222')) . "', 
	    ...
	    if($this->db->query($sql)){
			...
			$sql = "INSERT INTO student SET user_id = '".$last_id."',
			program_id ='". $this->db->escape($this->utf8_str_trim($data['program_id'])) . "', created_at=NOW()";
			if($this->db->query($sql)){
				return $last_id;
			}else{
				backToHome();
	...
	public function semseterCheck($program_id, $semester, $selected_list){
		if (array_key_exists($program_id, $selected_list)) {
			if (array_key_exists($semester, $selected_list[$program_id])) {
				return true;
			}
		}
	}	
	...
	public function createExam($data) {
		...
		$sems = $data['semester_select'];
		$sql = "INSERT INTO exams SET exam_date = '" . $this->db->escape($this->utf8_str_trim($data['exam_date']))."'";
		if($this->db->query($sql)){
			$exam_id = $this->db->getLastId();
			while ($sem = current($sems)) {
				$program_id = key($sems);
				while ($s = current($sem)) {
					$selsected_sem = key($sem);
					$sql = "INSERT INTO exam_structure SET exam_id = ".$exam_id.", program_id = " . $program_id. ", prog_semester = " . $selsected_sem;
					if(!$this->db->query($sql)){
						$this->db->error_log();
						return false;
					}
	...
	public function updateExam($data) {
		$data['exam_date'] = date('Y-m-d', strtotime($data['exam_date']));
		$sems = $data['semester_select'];
		$sql = "UPDATE exams SET exam_date = '".$this->db->escape($this->utf8_str_trim($data['exam_date']))."' WHERE exam_id = '".$data['exam_id']."'";
		if($this->db->query($sql)){
			$sql = "DELETE FROM exam_structure WHERE exam_id = ".$this->db->escape($this->utf8_str_trim($data['exam_id']));
			$this->db->query($sql);
			while ($sem = current($sems)) {
				$program_id = key($sems);
				while ($s = current($sem)) {
					$selsected_sem = key($sem);
					$sql = "INSERT INTO exam_structure SET exam_id = ".$data['exam_id'].", program_id = " . $program_id. ", prog_semester = " . $selsected_sem;
					if(!$this->db->query($sql)){
						$this->db->error_log();
						return false;
					}
	...
	public function getStudentDetailsById() {
		...
		$sql = "SELECT u.first_name, u.username, u.last_name, u.contact_no, s.program_id, p.program_code, p.program_name, p.no_of_semester, d.department_name  FROM users AS u 
		LEFT JOIN student AS s ON (u.user_id = s.user_id) 
		...
		$user_details = $this->db->query($sql);
		
		return $user_details->row;
	}
	
	public function getSubjectsUptoCurrentSemester($current_semester, $program_id) {
		
		$sql = "SELECT c.course_id, c.course_code, c.course_name, ps.semester FROM program_structure_credit AS ps
		LEFT JOIN courses AS c ON (ps.course_id = c.course_id) 
		 where ps.program_id='".$this->db->escape($program_id)."' AND ps.semester <= '".$this->db->escape($current_semester)."'";
		
		$course_details = $this->db->query($sql);
		return $course_details->rows;
	}

	public function getCourseStructureById($course_id) {
		
		$sql = "SELECT ps.course_id, ps.credit AS total_credit, ps.lecture, ps.tutorial, ps.practical, ps.project, p.fee_per_semester, p.program_id, ps.semester FROM program_structure_credit AS ps
		LEFT JOIN programs AS p ON (ps.program_id = p.program_id) 
		 where ps.course_id='".$this->db->escape($course_id)."'";
		$course_details = $this->db->query($sql);
		return $course_details->row;
	}

	public function totalCreditOfSemester($program_id, $semester) {
		$sql = "SELECT SUM(credit) AS total_creadit FROM program_structure_credit where program_id=".$this->db->escape($program_id)." AND semester = ".$this->db->escape($semester);
		$total_credit = $this->db->query($sql);
		return $total_credit->row['total_creadit'];
	}

	public function getAllActiveExams() {
		$sql = "SELECT exam_id, exam_date FROM exams WHERE application_status = 1 ORDER BY exam_date ASC";
		$query = $this->db->query($sql);
		return $query->rows;
	}

	public function apply_exam($data) {
		$user_id = $_SESSION['user_id'];
		$exam_id = $data['exam_id'];
		$sql = "INSERT INTO exam_applications SET user_id = '".$user_id."', exam_id = '".$exam_id."', total_fee = 3000, transaction_ref = '0', admitcard_status = '0'";

		if($query = $this->db->query($sql)){
			$application_id = $this->db->getLastId();		
			$details = array_slice($data,2);
			$data_chunks = array_chunk($details, 4, true);
			...
			foreach ($data_chunks as $key => $value) {
				...
				$sql = "INSERT INTO applications_to_course SET 	
				exam_application_id = '".$application_id."', 
				course_id = '".$value[$key]."', 
				type = '".$value[$i.',1']."', 
				exam = '".$exam."', 
				fee = '".$value[$i.',5']."'";
	...
	public function getApplicationById($application_id){
		$sql = "SELECT * FROM exam_applications where id = ".$application_id;
		return $this->db->query($sql)->row;
	}

	public function getAdmitcard($application_id, $user_id){
		
		$sql = "SELECT u.username, u.first_name, u.last_name, u.image, p.program_code, p.program_name FROM users AS u LEFT JOIN student AS s ON(u.user_id = s.user_id) LEFT JOIN programs AS p ON(s.program_id = p.program_id) where u.user_id = ";
		
		if($_SESSION['type'] == '2222'){
			$sql .= $_SESSION['user_id'];
		}else{
			$sql .= $user_id;
		}

		$student_details = $this->db->query($sql)->row;
		
		if(!empty($student_details)){
			$admitcard = array();
			$admitcard['student_details'] = $student_details;
			$sql = "SELECT e.exam_id, e.exam_date, ac.type, ac.exam, c.course_name
		 	FROM exams AS e LEFT JOIN exam_applications AS ea ON(e.exam_id = ea.exam_id)
		 	LEFT JOIN applications_to_course AS ac ON(ea.id = ac.exam_application_id)
			LEFT JOIN courses AS c ON(c.course_id = ac.course_id)
			 where ea.id = ".$application_id;
			 $data = $this->db->query($sql);
			 $admitcard['course_details'] = $data->rows;
			 return $admitcard;
		}
		return false;
	}

	public function getAppliedCoursesByAppId($application_id){
		$sql = "SELECT * FROM applications_to_course where exam_application_id = ".$application_id;
		return $this->db->query($sql)->rows;
	}

	public function getAllStudents(){
		$sql = "SELECT u.*, s.*, p.program_name FROM 
		users as u LEFT JOIN student as s ON (u.user_id=s.user_id) 
		LEFT JOIN programs as p ON (s.program_id=p.program_id) 
		where u.type = '2222'";
		return $this->db->query($sql)->rows;
	}

	public function searchStudentsByDetaile($data){ 	
   		$sql = "SELECT u.*, s.*, p.program_name FROM users as u LEFT JOIN student as s ON (u.user_id=s.user_id) LEFT JOIN programs as p ON (s.program_id=p.program_id) 
        WHERE u.type = '2222' ";
		if(!empty($data['first_name'])){
			$sql .= "AND u.first_name LIKE '%".$data['first_name']."%'  ";//GROUP BY be.book_id  HAVING min(be.book_entity_id)AND be.status = 1
		}
		if($data['program_id'] > '0'){
            $sql .= " AND s.program_id = '".$data['program_id']."'";
		}
	...
	public function getUserById($user_id){
		$sql = "SELECT u.*, s.*, p.program_name, p.program_code, d.department_name FROM 
		users as u LEFT JOIN student as s ON (u.user_id=s.user_id) 
		LEFT JOIN programs as p ON (s.program_id=p.program_id) 
		LEFT JOIN departments as d ON (p.department_id=d.department_id) 
		where u.type = '2222' AND u.user_id ='".$this->db->escape($this->utf8_str_trim($user_id))."'";
		return $this->db->query($sql)->row;
	}
	public function searchApplicationByDetaile($data){
		
		$sql = "SELECT ea.*, u.*, s.*, p.program_name FROM users as u
		LEFT JOIN student as s ON (u.user_id=s.user_id) 
		LEFT JOIN programs as p ON (s.program_id=p.program_id) 
		LEFT JOIN exam_applications as ea ON (ea.user_id=u.user_id) 
		WHERE u.type = '2222' ";
		if(!empty($data['first_name'])){
			$sql .= "AND u.first_name LIKE '%".$data['first_name']."%'  ";
		}
		if(!empty($data['exam_id'])){//!empty(data['exam_id']) exam_id id
			$sql .= "AND ea.exam_id = '".$data['exam_id']."'  ";
		}
		if($data['program_id'] > '0'){
			$sql .= " AND s.program_id = '".$data['program_id']."'";
		}
	...
	public function setAdmitcardDate($exam_id, $start_date, $end_date) {
		$start_date = date('Y-m-d', strtotime($start_date));
		$end_date = date('Y-m-d', strtotime($end_date));
		$sql = "UPDATE exams SET start_admitcard_date = '".$start_date."', end_admitcard_date = '".$end_date."' WHERE exam_id = '".$exam_id."'";
		$query = $this->db->query($sql);
		if($query){
			return true;
		}else{
	...
	public function getAllNotifications() {
		$sql = "SELECT id, text, created_at FROM notifications ORDER BY id ASC";
		$query = $this->db->query($sql);
		return $query->rows;
	}

	public function getNotificationById($notification_id) {
		$sql = "SELECT id, text FROM notifications where id =".$this->db->escape($notification_id);
		$result = $this->db->query($sql);
	...
	public function updateNotification($data) {
		$sql = "UPDATE notifications SET text = '".$this->db->escape($this->utf8_str_trim($data['notification_text']))."' WHERE id = '".$data['notification_id']."'";
		if($this->db->query($sql)){
			return true;
		}else{
			return false;
